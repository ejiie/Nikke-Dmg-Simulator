# T06 — 덱 후보 생성·샘플링 + background sim runner

> 필독: `CLAUDE.md` → `ROADMAP.md` → `FACTS.md` → 이 문서. 의존: T05
> (자동 sim 의 기본 tactic = T03 AutoController — T05 경유 전이 의존).
...
- 표본 우선순위(옵션): 유망 덱(부분 결과 고점) 집중 vs uniform 탐색 — MVP 는 uniform.
  T08 의 "표본 부족" 신호를 **선택적으로 수신**해 우선순위 조정 (신호 발신만 T08, 실행은 여기).
...
## 함정
- 덱 공간 폭발 (로스터 100명 → C(100,5) ≈ 7500만) — 전수 불가. **후보 덱 생성·샘플링·탐색 우선순위
  휴리스틱 = T06 소관** (2026-07-11 확정 — T08 은 기록된 후보의 선별 휴리스틱만).
