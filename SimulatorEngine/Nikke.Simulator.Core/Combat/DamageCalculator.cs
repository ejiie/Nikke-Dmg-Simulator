...
namespace Nikke.Simulator.Core.Combat
{
    ...
    ///   - <see cref="DamageCalculator"/>          : per-tick 대미지 공식 (이 클래스, Combat/)
    ///   - <c>Stats.StatCalculator</c>             : pre-combat 최종 기초스탯 조립 (레벨/돌파/코어/콘솔/장비)
    ///   - <c>Stats.OverloadProcessor</c>          : OL 합산 (니케식 정밀 소수점)
    ///   - <c>Stats.StatTable</c>                  : 원천 자료 로딩 (CSV/JSON) + raw 테이블 접근
    /// (이전엔 이 클래스가 `Stats/StatCalculator.cs` 에 있었고 이름이 역할과 괴리됐었음 — 대미지 계산 ≠ "Stat".)
    /// </summary>
    public static class DamageCalculator
    {
        ...
        /// 실측 완료: [gap#4] 비차지 무기 W·C 접힘 = in-game 검증됨(2026-06, 사용자). 잔여 대기: [gap#7] nested-vs-single floor 경계.
        /// </summary>
        ...
        public static double CalculateDamage(in AttackContext ctx)
        {
            ...
            //   [gap#4] 비차지 무기(AR/SMG/MG/SG) W·C 접힘 = in-game 실측 검증 완료(2026-06, 사용자).
